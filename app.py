...
def likert_with_skip(label: str, key: str, help_text: Optional[str] = None) -> Optional[int]:
    ...
    if skip:
        ...
        return None
    val = st.slider(label, min_value=0, max_value=4, key=key, help=help_text)
    return int(val)
...
def number_with_skip(label: str, key: str, placeholder: str = "") -> Optional[float]:
    # separate widget key so we never collide with feature names
    wkey = f"{key}__txt"
    c1, c2 = st.columns([1, 3])
    with c1:
        skip = st.checkbox("Prefer not to answer", key=f"{key}__skip")
    if skip:
        st.caption("↳ Skipped (blank for the model)")
        return None
    txt = st.text_input(label, key=wkey, placeholder=placeholder)
    txt = txt.strip()
    if txt == "":
        return None
    try:
...
which = st.sidebar.radio("Pages", PAGES, index=st.session_state.page, key="nav_pages")
new_page = PAGES.index(which)
...
# Consent checkbox outside forms (it won’t cause page reruns to jump)
...
if page == 0:
    ...
    with st.form("page0"):
        ...
        with c1:
            back0 = st.form_submit_button("Back", use_container_width=True)
        with c2:
            next0 = st.form_submit_button("Next", disabled=(not st.session_state.agree), use_container_width=True)
...
# Page 1: Demographics
    ...
    with st.form("page1"):
        ...
        c1, c2 = st.columns([1,1])
        with c1:
            back1 = st.form_submit_button("Back", use_container_width=True)
        with c2:
            next1 = st.form_submit_button("Next", use_container_width=True)
...
# Page 2: Smell & Bowel
    ...
    with st.form("page2"):
        ...
        with c2:
            next2 = st.form_submit_button("Next", use_container_width=True)
...
# Page 3: Urinary & Saliva
    ...
    with st.form("page3"):
        ...
        with c2:
            next3 = st.form_submit_button("Next", use_container_width=True)
...
# Page 4: Tremor & Balance
    ...
    with st.form("page4"):
        ...
        c1, c2 = st.columns([1,1])
        with c1:
            back4 = st.form_submit_button("Back", use_container_width=True)
        with c2:
            next4 = st.form_submit_button("Next", use_container_width=True)
...
# Page 5: Fatigue & Dizziness
    ...
    with st.form("page5"):
        ...
        c1, c2 = st.columns([1,1])
        with c1:
            back5 = st.form_submit_button("Back", use_container_width=True)
        with c2:
            next5 = st.form_submit_button("Next", use_container_width=True)
    ...
    with st.form("page6"):
        ...
        def zero_ten_with_skip(label, key):
            ...
            if skip:
                ...
                A[key] = None
                return
            A[key] = int(st.slider(label, 0, 10, key=wkey))
        ...
        c1, c2 = st.columns([1,1])
        with c1:
            back6 = st.form_submit_button("Back", use_container_width=True)
        with c2:
            next6 = st.form_submit_button("Next", use_container_width=True)
...
# Page 7: Review & Results
    ...
    with st.form("page7"):
        ...
        c1, c2 = st.columns([1,1])
        with c1:
            back7 = st.form_submit_button("Back", use_container_width=True)
        with c2:
            finish7 = st.form_submit_button("Finish", use_container_width=True, disabled=not st.session_state.agree)

    if back7:
        goto(page - 1)
    # Finish stays on same page; you can handle export here if needed
